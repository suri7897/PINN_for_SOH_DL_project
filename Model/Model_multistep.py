...
class PINN(nn.Module):
    ...
    ###* function for two_step learning 
    
    def train_one_solution_u(self,epoch,dataloader):
        self.train()
        loss1_meter = AverageMeter()
        loss2_meter = AverageMeter()
        for iter, (x1,x2,y1,y2) in enumerate(dataloader):
            x1, x2, y1, y2 = x1.to(device), x2.to(device), y1.to(device), y2.to(device)
            u1 = self.forward(x1)[0]
            u2 = self.forward(x2)[0]
    ...
    def train_one_dynamic_f(self,epoch,dataloader):
        self.train()
        loss3_meter = AverageMeter()
        for iter, (x1,x2,y1,y2) in enumerate(dataloader):
            x1, x2, y1, y2 = x1.to(device), x2.to(device), y1.to(device), y2.to(device)
            f1 = self.forward(x1)[1]
            f2 = self.forward(x2)[1]
            
            f_target = torch.zeros_like(f1)
            loss3 = 0.5*self.loss_func(f1,f_target) + 0.5*self.loss_func(f2,f_target)
            ...
            loss3_meter.update(loss3.item())
            if (iter+1) % 50 == 0:
                print("[epoch:{} iter:{}], PDE loss:{:.6f}".format(epoch,iter+1,loss3))
        
        return loss3_meter.avg
    

    def train_n_solution_u(self,n,epoch,dataloader):
        self.train()
        loss1_meter = AverageMeter()
        loss2_meter = AverageMeter()
        for i in range(n):
            for iter, (x1,x2,y1,y2) in enumerate(dataloader):
                ...
                loss2_meter.update(loss2.item())
                if (iter+1) % 50 == 0:
                    print("[epoch:{} iter:{}] data loss:{:.6f}, physics loss:{:.6f}".format(epoch+i,iter+1,loss1,loss2))
            self.scheduler.step()
        
        return loss1_meter.avg,loss2_meter.avg
    
    def train_n_dynamic_f(self,n,epoch,dataloader):
        self.train()
        loss3_meter = AverageMeter()
        for i in range(n):
            for iter, (x1,x2,y1,y2) in enumerate(dataloader):
                ...
                loss3_meter.update(loss3.item())
                if (iter+1) % 50 == 0:
                    print("[epoch:{} iter:{}], PDE loss:{:.6f}".format(epoch+i,iter+1,loss3))
    ...
    def Train(self,trainloader,testloader=None,validloader=None):
        ...
        early_stop = 0
        mae = 10
        for e in range(1,self.args.epochs+1, step):
            early_stop += step/2
            loss1,loss2 = self.train_n_solution_u(step, e,trainloader)
            loss3 = self.train_n_dynamic_f(step, e,trainloader)
            # loss1, loss2, loss3 = self.train_one_epoch(e,trainloader)
            current_lr = self.scheduler.get_lr()
            print('solution_u loss:{:.6f}, dynamic_F loss : {:.6f}'.format(loss1+self.alpha*loss2, loss3))
